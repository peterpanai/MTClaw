...
# memory_recall — wrapper for memory_engine.py recall command
...
QUERY=$(echo "$INPUT" | jq -r '.query | select(. != null)')
TAGS_JSON=$(echo "$INPUT" | jq -c '.tags | select(. != null)')
MTYPE=$(echo "$INPUT" | jq -r '.type | select(. != null)')
LIMIT=$(echo "$INPUT" | jq -r '.limit | select(. != null)')

# Validate required parameter
[ -z "$QUERY" ] && error_exit "missing required parameter: query"
