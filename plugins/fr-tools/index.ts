...
function pluginConfig(api: any): any {
  let fromGetter = {};
  try {
...
function resolveRootDir(config: any): string {
...
function readListenPort(rootDir: string): number {
  try {
...
function loadTools(rootDir: string, logger: any): any[] {
  try {
    ...
    try {
      ...
      logger.warn?.(
        `[fr-tools] no readable tool snapshot or functions.jsonl under ${rootDir}`,
      );
      return [];
    }
  }
...
function resultPayload(payload: any): any {
  return {
    content: [
      {
        type: "text",
...
function errorPayload(error: string): any {
  return resultPayload({ ok: false, error });
}

function errorMessage(error: any): string {
  if (error?.name === "AbortError") return "tool execution aborted or timed out";
  if (typeof error?.message === "string" && error.message) return error.message;
  return String(error || "tool execution failed");
}

async function executeRouterTool(
  routerUrl: string,
  name: string,
  params: any,
  signal: any,
