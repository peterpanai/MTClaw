...
function pickSessionKey(source: any): string {
  return typeof source?.sessionKey === "string" ? source.sessionKey.trim() : "";
}

function pickSessionId(source: any): string {
  return typeof source?.sessionId === "string" ? source.sessionId.trim() : "";
}

function buildSessionHeaders(source: any): Record<string, string> | null {
...
const sessionBridgePlugin = {
  ...
  register(api: any) {
    ...
    api.registerProvider({
       ...
       */
      resolveTransportTurnState(ctx: any) {
